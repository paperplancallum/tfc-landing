-- Add region column to airports table
...
-- Update the search_airports function to include region
...
BEGIN
  ...
  WHERE 
    ...
    OR LOWER(airports.region) LIKE LOWER('%' || search_term || '%')
  ORDER BY 
    CASE 
      ...
      WHEN LOWER(airports.region) LIKE LOWER(search_term || '%') THEN 6
      ELSE 7
    END,
    is_primary DESC,
    name;
...
-- Update the get_city_airports function to include region
...
-- Update the get_airport_by_code function to include region
